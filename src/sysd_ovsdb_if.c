...
sysd_set_hw_done(void)
{
    ...
    txn_status = ovsdb_idl_txn_commit_block(txn);
    if (txn_status != TXN_SUCCESS) {
        VLOG_ERR("Failed to set cur_hw, next_hw = 1. rc = %u\n", txn_status);
    }
    ovsdb_idl_txn_destroy(txn);
...
sysd_chk_if_hw_daemons_done(void)
{
    ...
    */
    ...
    VLOG_INFO("hostname:%s ret_code:%d Number of Daemons running %d", hostname,
              ret, num_hw_daemons);
...
sysd_run(void)
{
    ...
    if (new_seqno != idl_seqno) {
        ...
        if (cfg == NULL) {
            ...
            txn_status = ovsdb_idl_txn_commit_block(txn);
            if (txn_status != TXN_SUCCESS) {
                VLOG_ERR("Failed to commit the transaction. rc = %u\n", txn_status);
            }
            ovsdb_idl_txn_destroy(txn);
        } else {
