...
sysd_add_package_info(struct ovsdb_idl_txn *status_txn)
{
    ...
    while (getline(&file_line, &line_len, ver_detail_fp) != -1) {
        ...
        if (p_field && ((p_field + strlen("PKG=")) - name < PKG_INFO_MAX_LEN)) {
            p_field += strlen("PKG=");
        ...
        if (NULL == p_field) {
            ...
            if (p_field && ((p_field + strlen("SRCREV=")) - git_hash < PKG_INFO_MAX_LEN)) {
                p_field += strlen("SRCREV=");
                ovsrec_package_info_set_version(row, p_field);
            }
        } else {
            p_field = strstr(version, "PV=");
            if (p_field && ((p_field + strlen("PV=")) - version < PKG_INFO_MAX_LEN)) {
                p_field += strlen("PV=");
                ovsrec_package_info_set_version(row, p_field);
            }
        }
        ...
        if (p_field && ((p_field + strlen("TYPE=")) - type < PKG_INFO_MAX_LEN)) {
            p_field += strlen("TYPE=");
        ...
        if (p_field && ((p_field + strlen("SRC_URL=")) - src_url < PKG_INFO_MAX_LEN)) {
