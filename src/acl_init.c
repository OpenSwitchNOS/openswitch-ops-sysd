               ...
               struct ovsrec_system *system_row)
{
    ...
    struct smap smap;
    char max_acls_str[16];
    char max_aces_str[16];
    char max_aces_per_acl_str[16];

    acl_info = sysd_cfg_yaml_get_acl_info();

    /* smap_replace expects char values */
    snprintf(max_acls_str, sizeof(max_acls_str), "%d", acl_info->max_acls);
    snprintf(max_aces_str, sizeof(max_aces_str), "%d", acl_info->max_aces);
    snprintf(max_aces_per_acl_str, sizeof(max_aces_per_acl_str), "%d", acl_info->max_aces_per_acl);
