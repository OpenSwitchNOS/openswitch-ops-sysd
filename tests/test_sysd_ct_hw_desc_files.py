...
class HWDescTest(OpsVsiTest):
    ...
    This class will be instanciated by the py.test TestRunner below.
    """
    ...
    def check_max_bond_member_count(self):
        ...
        assert count == str(self.expected['port_info']['max_lag_member_count']),\
    ...
    def check_switch_intf_id(self):
        ...
        assert intf_id == str(self.expected['ports'][0]['switch_device_port']),\
    ...
    def check_connector_type(self):
        ...
        assert connector_type == str(self.expected['ports'][0]['connector']),\
    ...
    def check_default_bridge_name(self):
        ...
        name = name.replace('\r\n','')
        assert name == "bridge_normal", "Default bridge is not created."

    def check_default_vrf_name(self):
        name = self.s1.ovscmd(OVS_VSCTL + "get vrf vrf_default name")
        name = name.replace('\r\n','')
        assert name == "vrf_default", "Default VRF is not created."
...
class TestRunner:
    """py.test based test runner class for test_sysd_ct_os_release.py."""
